...
export const deleteProductReview =(review , productid)=>(dispatch , getState)=>{

  dispatch({type : "DELETE_PRODUCT_REVIEW_REQUEST"})

  const currentUser = getState().loginReducer.currentUser
  axios.post('/api/products/deletereview',{review , productid, currentUser}).then(res=>{
    console.log(res)

    dispatch({type : "DELETE_PRODUCT_REVIEW_SUCCESS" , payload : res.data})
    alert("Your review deleted Successfully")
    window.location.reload()
  }).catch((err) => {
    dispatch({ type: "DELETE_PRODUCT_REVIEW_FAILED" , payload : err});
  });
};
...
export const addorder =(product)=> dispatch=>{
  ...
  axios.post('/api/orders/addorder' , {product}).then(res=>{
    console.log(res);
    dispatch({type:'ADD_PRODUCT_SUCCESS'})
    window.location.reload()
  }).catch(err=>{
